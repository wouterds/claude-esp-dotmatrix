...
describe("readLimits", () => {
  it("reports nothing rather than nothing-used when the file is not there yet", async () => {
    expect(await readLimits(NOW, "/nope/limits.json")).toEqual(NO_LIMITS);
  });
  ...
  it("falls back rather than throwing on a file that is not json", async () => {
    ...
    expect(await readLimits(NOW, file)).toEqual(NO_LIMITS);
  });
  ...
  // The whole reason a stale file is safe to trust: within one window a quota
  // only ever grows, so a reading nothing has invalidated is still the number.
  it("keeps a reading whose window has not rolled, however old the file is", async () => {
    ...
    expect((await readLimits(NOW, file)).fiveHour?.used).toBeCloseTo(0.6);
  });

  // And the other side of it: past the reset the window has started again and
  // the last reading describes a window that is gone. Better a dark row than a
  // number that is wrong in the direction of looking fine.
  ...
  it("carries the reset through, so a tool can say when the window lifts", async () => {
    ...
    expect((await readLimits(NOW, file)).fiveHour?.resetsAt).toBe(NOW + HOUR);
  });
});
